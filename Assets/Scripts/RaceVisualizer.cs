...
public class RaceVisualizer : MonoBehaviour
{
    ...
    public void StartRace()
    {
        if (!isVisualizing && raceData != null && raceData.Count > 0)
        {
            isVisualizing = true;
            currentFrame = 0;
            StartCoroutine(VisualizeRaceData());
            Debug.Log("Started F1 race visualization.");
        }
    }
    ...
    private IEnumerator VisualizeRaceData()
    {
        while (isVisualizing)
        {
            ...
            Dictionary<int, Vector3> frameData = raceData[currentFrame];
            
            // Log frame information
            Debug.Log($"Frame {currentFrame} of {totalFrames - 1}:");
            
            // Process each car in the frame
            foreach (var carEntry in frameData)
            {
                ...
                // Here you would typically update game objects representing the cars
    ...
    // Example method to update car position (implement as needed)
